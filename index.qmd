...
source("./R/generate_instruction.R")
try({
...
if (!knitr::is_html_output()) {
  p1 <- generate_mandala(type = "example")
  p2 <- generate_instruction("Mandala", "Color the mandala according to the example provided.")
  p3 <- generate_sudoku(NULL, type = "example")
  p4 <- generate_instruction("Sudoku", "Fill in the blank squares so that each row, each column and each\n3-by-3 block contains all of the digits 1 to 9.")
  p5 <- generate_wordfinder(NULL, wordlist, type = "example")
  p6 <- generate_instruction("Word Finder", "Locate and strike through all words in the puzzle. Words may\nappear vertically, horizontally, or diagonally. Note that letters\ncan be part of multiple words.")
  p7 <- generate_crossword(NULL, wordlist, type = "example")
  p8 <- generate_instruction("Crossword", "Fill in the synonyms of the words in the list at their corresponding\npositions in the puzzle.")
  gridExtra::grid.arrange(p1, p2, p3, p4, p5, p6, p7, p8, layout_matrix = matrix(c(
    1, 2, 2,
    4, 4, 3,
    5, 6, 6,
    8, 8, 7
  ), byrow = TRUE, nrow = 4, ncol = 3), top = grid::textGrob("— Instructions —", gp = grid::gpar(fontsize = 75, fontfamily = getOption("book.font.type", "sans"), fontface = "bold")))
}
```
\clearpage
...
if (!knitr::is_html_output()) {
  ...
  p1 <- generate_mandala()
  set.seed(seed)
  p2 <- generate_mandala(type = "solution")
...
if (!knitr::is_html_output()) {
  ...
  for (i in 1:9) {
    ...
    plotList[[length(plotList) + 1]] <- generate_sudoku(sudokuseeds[i], type = "solution")
  }
...
if (!knitr::is_html_output()) {
  ...
  for (i in 1:9) {
    ...
    plotList[[length(plotList) + 1]] <- generate_wordfinder(wordfinderseeds[i], wordlist, type = "solution")
  }
...
if (!knitr::is_html_output()) {
  ...
  for (i in 1:9) {
    ...
    plotList[[length(plotList) + 1]] <- generate_crossword(crosswordseeds[i], wordlist, type = "solution")
  }
