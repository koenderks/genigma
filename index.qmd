...
```{r mandal-2, echo = FALSE, message=FALSE}
if (!knitr::is_html_output() && !debug) {
  seed <- mandalaseeds[2]
  set.seed(seed)
  p1 <- mandala()
...
```{r nongram-12, echo = FALSE, message=FALSE}
if (!knitr::is_html_output() && !debug) {
...
```{r crosswrod-16, echo = FALSE, message=FALSE}
if (!knitr::is_html_output() && !debug) {
...
if (!knitr::is_html_output() && !debug) {
  ...
  gridExtra::grid.arrange(p1, layout_matrix = matrix(1:9, byrow = TRUE, nrow = 3, ncol = 3), top = grid::textGrob("— Nonogram ~ Solutions —", gp = grid::gpar(fontsize = 75, fontfamily = getOption("book.font.type", "sans"), fontface = "bold")))
}
```
\clearpage
